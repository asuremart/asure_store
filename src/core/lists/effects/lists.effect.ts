...
export class ListsEffect {
  ...
  @Effect()
  widgetDetail$: Observable<Action> = this.actions$.pipe(
    ofType(actions.ActionTypes.WIDGET_DETAIL),
    map((action: actions.widgetDetails) => action.payload),
    switchMap(state => {
      return this.authApi.getWidgetDetail(state).pipe(
        map(register => new actions.widgetDetailsSuccess(register)),
        catchError(error => of(new actions.widgetDetailsFail(error)))
      );
    })
  );

  @Effect()
  widgetDetailCount$: Observable<Action> = this.actions$.pipe(
    ofType(actions.ActionTypes.WIDGET_DETAIL_COUNT),
    map((action: actions.widgetDetailsCount) => action.payload),
    switchMap(state => {
      return this.authApi.getWidgetDetailCount(state).pipe(
        map(register => new actions.widgetDetailsCountSuccess(register)),
        catchError(error => of(new actions.widgetDetailsCountFail(error)))
      );
    })
  );
