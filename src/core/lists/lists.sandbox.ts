...
import {
  ...
  widgetDetailLoaded,
  widgetDetailFailed,
  widgetDetailCountLoading,
  widgetDetailCountLoaded,
  widgetDetailCountFailed,
  widgetDetailCount
...
export class ListsSandbox {
  ...
  public widgetDetail$ = this.appState$.select(widgetDetail);
  public widgetDetailLoading$ = this.appState$.select(widgetDetailLoading);
  public widgetDetailLoaded$ = this.appState$.select(widgetDetailLoaded);
  public widgetDetailFailed$ = this.appState$.select(widgetDetailFailed);

  public widgetDetailCount$ = this.appState$.select(widgetDetailCount);
  public widgetDetailCountLoading$ = this.appState$.select(widgetDetailCountLoading);
  public widgetDetailCountLoaded$ = this.appState$.select(widgetDetailCountLoaded);
  public widgetDetailCountFailed$ = this.appState$.select(widgetDetailCountFailed);
  ...
  public widgetDetail(params): void {
    this.appState$.dispatch(
      new authAction.widgetDetails(params));
  }

  public widgetDetailCount(params): void {
    this.appState$.dispatch(
      new authAction.widgetDetailsCount(params));
  }
  ...
  public widgetDiscount() {
    this.widgetProductList$.subscribe(res => {
      if (res && res.data && res.data.length > 0) {
        res.data.forEach(product => {
          if (product.items) {
            const tempProduct = product.items.map(data => {
              if (data) {
